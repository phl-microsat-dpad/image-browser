...
angular.module('dauriaSearchApp')
  .controller('MainCtrl', ['$scope', '$filter', 'leafletData', 'leafletBoundsHelpers', '$http', '$sce', '$q', '$modal', function($scope, $filter, leafletData, leafletBoundsHelpers, $http, $sce, $q, $modal) {
    ...
    var storage_api = 'https://phl-microsat.dream.upd.edu.ph/storage-api/scenes/';
    ...
    $scope.execQuery = function() {
      ...
      var neLat = mod($scope.bounds.northEast.lat + 180, 360) - 180;
      var neLong = mod($scope.bounds.northEast.lng + 180, 360) - 180;
      ...
      if(swLat  < MIN_LAT){ swLat = MIN_LAT; }
      if(neLat  > MAX_LAT){ neLat = MAX_LAT; }
      if(neLong > MAX_LONG){ neLong = MAX_LONG; }

      console.log('New Coordinates: SW: ' + swLat + ', ' + swLong + ' | NW: ' + neLat + ', ' + neLong);


      $scope.searchString = queryConstructor({
        dateRange: [$scope.dateRangeStart, $scope.dateRangeEnd],
        limit: 3000,
        sceneCenterLatRange: [neLat, swLat],
        sceneCenterLonRange: [neLong, swLong],
        continuous: continuous
      });

      if(neLong < MIN_LONG || neLat < MIN_LAT || swLong > MAX_LONG || swLat > MAX_LAT) {
          console.log('Out of bounds!');
          $scope.results = [];
          var msg = 'Out of bounds! You may only browse images within the Philippines';
    ...
    $scope.getDownloadSize = function(result) {
      if (!result.downloadSize) {
        ...
        .success( function(data){
          ...
          result.noData = false;
          result.downloadSize = ' (' + Math.round(data.bundle_size / 1048576) + ' MB)';
        })
        .error( function(){
